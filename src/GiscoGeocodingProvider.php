...
namespace OpenEuropa\Provider\GiscoGeocoding;
 ...
 */
class GiscoGeocodingProvider extends AbstractHttpProvider
{
    /**
     * @var string
     */
    ...
    {
        throw new UnsupportedOperation('The GISCO Geocoding Provider does not support reverse geocoding.');
    }
     ...
     *   The location feature JSON object, as returned by the GISCO Geocoding
     *   REST API.
     *
     * @return array|null
    ...
    protected function getAddressData(\stdClass $feature): ?array
    {
        ...
        if (
            isset($feature->properties->extent) && is_array(
                $feature->properties->extent
            ) && count($feature->properties->extent) === 4
        ) {
            $addressData['bounds'] = [
